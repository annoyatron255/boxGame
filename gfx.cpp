...
namespace gfx {
	int gfxinit(int w,int h) {
		if( SDL_Init( SDL_INIT_VIDEO ) < 0 ) {
			...
			SDL_SetHint( SDL_HINT_RENDER_SCALE_QUALITY, "0" ); // Remove filtering
			/*if( !SDL_SetHint( SDL_HINT_RENDER_SCALE_QUALITY, "1" ) ) {
				printf( "Warning: Linear texture filtering not enabled!" );
			}*/
			m_window = SDL_CreateWindow( "boxGame for two players", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, w, h, SDL_WINDOW_SHOWN /*| SDL_WINDOW_FULLSCREEN*/ );
			if(m_window == NULL) {
